...
["webkit-3d","moz-webgl","webgl","experimental-webgl"];for(let g=0;g<e.length;g++)try{const f=a.getContext(e[g],b);return new Program(f,c,d)}catch(f){}return null}getAttribsAndUniforms(a,b,c,d){a=a.split("\n");for(let l=0;l<a.length;l++){var e=a[l].trim();if("uniform"===e.substr(0,7)){e=e.split(" ");var g=e[1];for(var f=2;f<e.length;f++){var h=e[f];if(";"===h)break;var k=h.charAt(h.length-1);const m=";"===k;if(","===k||m)h=h.substr(0,h.length-1);","!==h&&h.length&&(c.push(h),d.push(g));if(m)break}}else if("attribute"===
